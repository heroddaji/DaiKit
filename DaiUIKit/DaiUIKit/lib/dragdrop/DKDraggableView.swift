...
//  DressRoom
//
//  Created by daitran on 5/21/15.
//  Copyright (c) 2015 daitran. All rights reserved.
//

import UIKit

protocol DKDraggableViewDelegate{
    func onDropedToTarget(sender: DKDraggableView, target: UIView)
}

class DKDraggableView: UIImageView, UIGestureRecognizerDelegate {
    ...
    var delegate:DKDraggableViewDelegate?
    
    private var isEnteredDropTarget = false
    
    @IBInspectable var enableDragging = false {
        didSet{
    ...
    func createFakeDragginView() {
        ...
        draggingView?.addSubview(UIImageView(image: DKHelper.scaledImageToSize(self.image!, newSize: self.bounds.size)))
        self.superview?.addSubview(draggingView!)
        
    }
    
    func removeFakeDraggingView(){
    ...
    //MARK: drop target
    func setDropTarget(target: UIView){
        dropTarget = target
    }
